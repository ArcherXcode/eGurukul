...
const TestAndQuizzesScreen = () => {
  ...
  const testsAndQuizzes = [
    ...
    { id: 3, name: 'Physics Test', status: 'past', code: 'PHYS101', teacher: 'Mr. Smith', date: '2024-11-10', type: 'Test', marks: 20, totalmarks: 30 },
  ...
  const beginTest = (id: number, name: string, code: string, teacher: string, date: string) => {
    router.push(`../test/detail?id=${id}&name=${name}&code=${code}&teacher=${teacher}&date=${date}`);
  }
        ...
        {ongoingTests.length > 0 && (
          <>
            {ongoingTests.map((test) => (
              ...
              onPress={() => beginTest(test.id, test.name, test.code, test.teacher, test.date)}
              >
              ...
              <View key={test.id} style={styles.testCard}>
                <View style={[styles.pastUpcomingTest]}>
                  ...
                  {test.marks !== undefined && (
                    <Text style={styles.testDetails}>Marks: {test.marks}/{test.totalmarks}</Text>
                  )}
                  <View style={styles.statusDot}>
