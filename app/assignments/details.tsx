...
const Details = () => {
  ...
  const Actions = [
    ...
    },
    {
      ...
      color: '#1e90FF',
      buttonSize: 60,
      icon: <Entypo name='link' size={24} color='#fff' />,
      textStyle: { fontSize: 18, fontWeight: '500' },
    },
    {
      text: "Create Test",
  ...
  const handleAction = (name: string) => {
    ...
    if (name === 'submit') {
      ...
      router.push({ pathname: `/assignments/create?id:${assID}&name=${assName}&deadline=${assDeadline}` });
    }
    else if (name === 'test') {
      router.push({ pathname: `/test/createtest?id:${assID}&name=${assName}&deadline=${assDeadline}` });
    }
  };
